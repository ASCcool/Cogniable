...
def get_model_flow(a = ''):
    print(a)
    blob_list = bucket.list_blobs(name_starts_with="Final_Binary_Classification_Dataset/Flow_Model_28Nov_lre-3/")
    path = ""
    for blob in blob_list:      
        if (a != '') and blob.name == f"Final_Binary_Classification_Dataset/Flow_Model_28Nov_lre-3/FlowModel_{a}.pkl":
          path = blob.name
          print(path)
          break
    if path == "":
...
for epoch in range(200):  # loop over the dataset multiple times
    ...
    dump("FlowModel.pkl",f"Final_Binary_Classification_Dataset/Flow_Model_28Nov_lre-3_2/FlowModel_{offset + epoch + 1}.pkl",flow)
    dump_pickle("FlowModelLoss.pkl",f"Final_Binary_Classification_Dataset/Flow_Model_28Nov_lre-3_2/FlowModel_Loss_{offset + epoch + 1}.pkl",loss)
    #plot_loss(t)

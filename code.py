...
#def get_data(a):
 #   path = "" 
  #  path = f"ai_team/rgb_data/Data_v2/Train_data/train_{a}.pkl"
   # temp1 = I3D(num_classes= 7)
    #temp1 = torch.nn.DataParallel(temp1)
    #temp1 = torch.load(io.BytesIO(bucket.download_blob(path).readall()))
    #return temp1
dataset = []
...
train_set = int(0.7 * len(dataset)) # 70% for training
val_set = len(dataset) - train_set # 30% for validation
...
def get_model_flow(a = ''):
    blob_list = bucket.list_blobs(name_starts_with="Final_Binary_Classification_Dataset/Flow_Model_V6/")
    print(a)
    path = ""
    for blob in blob_list:      
        if (a != ''):
            if blob.name == f"Final_Binary_Classification_Dataset/Flow_Model_V6/FlowModel_{a}.pkl":
                path = blob.name
                print(path)
                break
    if path == "":
   ...
   #temp1.load_state_dict(torch.load(io.BytesIO(bucket.download_blob(path).readall())))
